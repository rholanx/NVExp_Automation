...
# Import ingestion utilities with fallback for direct execution
try:
    ...
    try:
        ...
        # Fallback for running from repository root or different CWDs
...
try:
    from ..db_utils import initialize_database, close_database, db_pool
    ...
    try:
        ...
        from KG.models import IngestionConfig, IngestionResult
...
class DocumentIngestionPipeline:
    ...
    def __init__(
        ...
        update_graph_only: bool = False,   # <<< NEW
    ):
        """
        ...
        Args:
            ...
            update_graph_only: If True, update KG only (do not write to PostgreSQL)
        """
        self.config = config
        self.documents_folder = documents_folder
        self.clean_before_ingest = clean_before_ingest
        self.update_graph_only = update_graph_only  # <<< NEW
    ...
    async def initialize(self):
        ...
        if not self.update_graph_only:
            await initialize_database()
        else:
            logger.info("[Graph-Only] Skipping PostgreSQL initialization")
    ...
    async def close(self):
        ...
        if self._initialized:
            ...
            if not self.update_graph_only:
                ...
                logger.info("[Graph-Only] Skipping PostgreSQL close")
            self._initialized = False
    ...
    async def ingest_documents(
        ...
        # Find all document files (markdown, text, log)
        ...
        logger.info(f"Found {len(markdown_files)} document files to process")
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        # Generate embeddings (graph may leverage embeddings; keep step even in graph-only)
        embedded_chunks = await self.embedder.embed_chunks(chunks)
        logger.info(f"Generated embeddings for {len(embedded_chunks)} chunks")
        
        # Save to PostgreSQL (skip when graph-only)
        if self.update_graph_only:
            document_id = ""
            logger.info("[Graph-Only] Skipping PostgreSQL writes; proceeding to KG update")
        else:
    ...
    def _find_markdown_files(self) -> List[str]:
        """Find all markdown, text, and log files in the documents folder."""
        if not os.path.exists(self.documents_folder):
            logger.error(f"Documents folder not found: {self.documents_folder}")
            return []
        
        patterns = ["*.md", "*.markdown", "*.txt", "*.log"]
        files = []
    ...
    async def _clean_databases(self):
        ...
        # Clean PostgreSQL (skip when graph-only)
        if self.update_graph_only:
            logger.info("[Graph-Only] Skipping PostgreSQL cleanup")
        else:
            async with db_pool.acquire() as conn:
                async with conn.transaction():
...
async def main():
    ...
    parser.add_argument("--documents", "-d", default="./KG/documents", help="Documents folder path")
    ...
    parser.add_argument("--graph-only", action="store_true",
                        help="Update knowledge graph only (skip PostgreSQL reads/writes/init/cleanup)")  # <<< NEW
    ...
    pipeline = DocumentIngestionPipeline(
        ...
        update_graph_only=args.graph_only,  # <<< NEW
    )
...
if __name__ == "__main__":
    """
    Usage: 
    1. Graph only (don’t touch PostgreSQL):
    python KG/ingestion/ingest.py --graph-only
    2. Normal full ingestion (DB + KG):
    python KG/ingestion/ingest.py
    3. Graph only + skip graph building (fast no-op for KG edges, useful for testing):
    python ingest.py --graph-only --fast
    4. Full ingestion + skip graph building (fast no-op for KG edges, useful for testing):
    python ingest.py --fast
    5. Full ingestion + skip graph building + skip entity extraction (fast no-op for KG edges, useful for testing):
    python ingest.py --fast --no-entities
    6. Full ingestion + skip graph building + skip entity extraction + skip semantic chunking (fast no-op for KG edges, useful for testing):
    python ingest.py --fast --no-entities --no-semantic
    7. Full ingestion + skip graph building + skip entity extraction + skip semantic chunking + skip frontmatter extraction (fast no-op for KG edges, useful for testing):
    """
    asyncio.run(main())
